...
// Price inflation multiplier (raw prices × multiplier)
let priceMultiplier = store.get('bvs.priceMultiplier', null);
...
function renderGuide(){
  ...
  for(const item of filtered){
    ...
    const doses = (item.doses||[]).map(label=>{
      ...
      const badge = stock==null ? '' : `<span class="stock-badge ${cls}">${out?'Out':(stock<=5?'Low':'In')} • ${stock}</span>`;
      return `
...
function computePrices(){
  // PRICES_RAW: [name, strength, price, unit, stock]
  return (PRICES_RAW||[]).map(row=>{
    const [name, strength, price, unit, stock] = row;
    const {amount, unit: parsedUnit} = parseAmount(strength);
    const u = (String(unit||parsedUnit||'').toLowerCase() === 'iu') ? 'IU' : 'mg';
    const amt = isFinite(amount) ? amount : NaN;
    const pRaw = Number(price)||0;
    const mult = (priceMultiplier == null) ? 1 : Number(priceMultiplier);
    const p = pRaw * (isFinite(mult) ? mult : 1);
    const s = Number(stock)||0;
    const ppu = (isFinite(amt) && amt>0) ? (p/amt) : NaN;
    return {name, strength, amount:amt, price:p, priceRaw:pRaw, ppu, stock:s, unit:u};
  });
}
  ...
  for(const r of rows){
    ...
    tbody.insertAdjacentHTML('beforeend', `
      <tr>
        ...
        <td class="num" title="Raw ${money(r.priceRaw)} × ${Number(priceMultiplier??1).toFixed(2)}">${money(r.price)}</td>
...
async function loadData(){
  ...
  // If user hasn't set a multiplier yet, seed from data.json default (or 1.0)
  if(priceMultiplier == null){
    const d = Number(DATA.price_multiplier_default);
    priceMultiplier = isFinite(d) ? d : 1.0;
    store.set('bvs.priceMultiplier', priceMultiplier);
  }
}
...
function wirePrices(){
  ...
  const multInput = $('#pmult');
  if(multInput){
    // initialize UI
    multInput.value = String(Number(priceMultiplier ?? 1).toFixed(2));
    multInput.addEventListener('input', ()=>{
      const v = Number(multInput.value);
      priceMultiplier = isFinite(v) ? v : 1;
      store.set('bvs.priceMultiplier', priceMultiplier);
      renderPrices();
    });
  }

...
function renderGuide(){
  ...
  function renderCard(item, targetGrid){
    ...
    const prices = (item.doses || [])
      .map(label => getPrice(item.name, label))
    ...
    const html = `
      <article class="card ${item.isKitCard ? 'kit-card' : ''}" data-key="${esc(item.key)}" data-modal-name="${esc(item.name)}" data-modal-badge="${esc(item.badge||'')}" data-modal-more="${esc(item.more || '')}">
        ...
        <div class="mini-wrap" data-mini="${esc(item.key)}" aria-label="Mini calculator" style="display:none">
          <div class="mini-head">
            <div>
          ...
          <div class="mini-body">
            <div class="mini-form" role="group" aria-label="Peptide calculator inputs">
              ...
              <div class="mini-field mini-unit field-syringe">
                <label>Syringe</label>
            ...
            <div class="mini-stats">
              <div class="stat"><b>Concentration</b><span data-role="conc">—</span></div>
              <div class="stat"><b>Draw</b><span data-role="draw">—</span></div>
              <div class="stat"><b>Units</b><span data-role="units">—</span></div>
            </div>

            <div class="syringe-wrap">
              <div data-role="syringeVisual"></div>
            </div>

            <div class="warn" data-role="warning" style="display:none"></div>
          </div>
        </div>
...
function openMini(card, btn, doseLabel){
  ...
  const parking = card.querySelector(`[data-mini-parking="${CSS.escape(card.dataset.key || '')}"]`) || card.querySelector('.mini-parking');
  if(parking) parking.appendChild(mini);

  const parsed = parseAmount(doseLabel);
  mini.dataset.vialUnit = parsed.unit || 'mg';

  const amountInput = mini.querySelector('[data-role="amountPerVial"]');
  const amountUnitSel = mini.querySelector('[data-role="amountUnit"]');
  ...
  if(amountInput && Number.isFinite(parsed.amount)) amountInput.value = String(parsed.amount);
  if(amountUnitSel) amountUnitSel.value = parsed.unit === 'iu' ? 'iu' : 'mg';
  if(diluentInput) diluentInput.value = '1';
  if(desiredInput) desiredInput.value = desiredInput.value || '0';
  if(desiredUnitSel){
    desiredUnitSel.value = parsed.unit === 'iu' ? 'iu' : 'mg';
  }
  if(metaEl){
    metaEl.textContent = doseLabel ? `${doseLabel} selected` : 'Selected';
  }
...
function renderSyringeVisual(container, scale, unitsPct){
  if(!container) return;
  container.innerHTML = `
    <div class="classic">
      <div class="tube">
        <div class="fill" style="width:${unitsPct}%"></div>
        <div class="cursor" style="left:calc(${unitsPct}% - 1px)"></div>
      </div>
      <div class="ticks"></div>
    </div>
  `;
  const ticks = container.querySelector('.ticks');
  if(ticks) ensureTicks(ticks, scale);
}
...
function updateMini(mini){
  ...
  const desiredUnit = mini.querySelector('[data-role="desiredUnit"]')?.value || 'mg';
  const syringeScale = Number(mini.querySelector('[data-role="syringeType"]')?.value || 100);
  const vialUnit = mini.querySelector('[data-role="amountUnit"]')?.value || mini.dataset.vialUnit || 'mg';
  ...
  const noteEl = mini.querySelector('[data-role="warning"]');
  const syringeVisual = mini.querySelector('[data-role="syringeVisual"]');

  if(concEl){
    concEl.textContent = (amountPerVial > 0 && diluentMl > 0) ? `${num(amountPerVial / diluentMl)} ${vialUnit}/mL` : '—';
  }
  if(drawEl){
    drawEl.textContent = Number.isFinite(res.units) ? `${num(res.units * 0.01)} mL` : '—';
  }
  if(unitsEl){
    unitsEl.textContent = Number.isFinite(res.units) ? `${num(res.units)} units` : '—';
  }
  if(noteEl){
    if(res.note){
      noteEl.style.display = 'block';
      noteEl.textContent = res.note;
    }else if(Number.isFinite(res.units) && res.units > syringeScale){
      noteEl.style.display = 'block';
      noteEl.textContent = 'Heads up: draw exceeds the selected syringe scale.';
    }else{
      noteEl.style.display = 'none';
      noteEl.textContent = '';
    }
  }
  renderSyringeVisual(syringeVisual, syringeScale, Number.isFinite(res.unitsPct) ? res.unitsPct : 0);
}
...
function wireGuide(){
  ...
  grid.addEventListener('click', (e)=>{
    ...
    if(doseBtn){
      ...
      if(card) openMini(card, doseBtn, doseBtn.dataset.dose || doseBtn.textContent || '');
      return;
    }
    ...
    if(presetBtn){
      const mini = presetBtn.closest('.mini-wrap');
      const desiredInput = mini?.querySelector('[data-role="desiredDose"]');
      if(mini && desiredInput){
        desiredInput.value = presetBtn.dataset.preset || '0';
        updateMini(mini);
      }
      return;
    }
